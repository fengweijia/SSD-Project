...
std::vector<Prediction> caffe_classifier::Classify(const cv::Mat& img, int N)
{
 ...
 * layer.
 */
void caffe_classifier::WrapInputLayer(std::vector<cv::Mat>* input_channels)
{
  Blob<float>* input_layer = net_->input_blobs()[0];
...
std::vector<Prediction> caffe_classifier::BlacklungClassify(cv::Mat& img)
{
    CHECK(!img.empty()) << "Unable to decode image" << std::endl;
    std::vector<Prediction> predictions = Classify(img);

    std::cout << "Caffe models start classifying images....." << std::endl;
